...
use crate::graph::{DSEdgeID, PortChannelID, PortType};
...
pub(super) fn sync_ports_in_graph(
    ...
    let mut status = Ok(());
    if let Some(audio_ports) = &plugin_host.save_state.backup_audio_ports_ext {
    ...
    id_alias_check.clear();
    if let Some(note_ports) = &plugin_host.save_state.backup_note_ports_ext {
